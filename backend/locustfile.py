...
class EcommerceUser(HttpUser):
    ...
    def wishlist_operations(self):
        ...
        # Get Wishlist - Endpoint: GET /wishlist/
        self.client.get("wishlist/", headers=self.headers, name="/wishlist/")

        # Remove item - Endpoint: DELETE /wishlist/remove/{product_id}
        if random.random() < 0.2:
            self.client.delete(f"wishlist/remove/{prod_id}", headers=self.headers, name="/wishlist/remove/[prod]")

        # Move to cart - Endpoint: POST /wishlist/move-to-cart
        self.client.post(f"wishlist/move-to-cart?product_id={prod_id}", headers=self.headers, name="/wishlist/move-to-cart")

        # Clear - Endpoint: DELETE /wishlist/clear
        if random.random() < 0.1:
            self.client.delete("wishlist/clear", headers=self.headers, name="/wishlist/clear")
    ...
    def address_lifecycle(self):
        ...
        if res.status_code == 201:
            # The backend returns SuccessResponse(message="Address added")
            # We need to fetch addresses to get the address_id for update/delete
            addr_res = self.client.get("address/all", headers=self.headers)
            if addr_res.status_code == 200:
                addresses = addr_res.json()["data"]
                if addresses:
                    addr_id = addresses[-1]["address_id"]
                    self.address_ids.append(addr_id)

                    # Update - Endpoint: PUT /address/update/{address_id}
                    self.client.put(f"address/update/{addr_id}", json={
                        "street": f"{random.randint(1, 999)} Updated St",
                        "city": "Updated City",
                        "state": "Updated State",
                        "country": "Updated Country",
                        "zipcode": "654321"
                    }, headers=self.headers, name="/address/update/[id]")
    ...
    def cleanup_resources(self):
        ...
        # Delete Address - Endpoint: DELETE /address/delete/{address_id}
        if self.address_ids:
            addr_id = self.address_ids.pop()
            self.client.delete(f"address/delete/{addr_id}", headers=self.headers, name="/address/delete/[id]")
